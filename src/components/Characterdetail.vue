<template>
  <div>
    ...
    <div v-else>
    <div v-if="err">Error 404 : Character not found</div>
    <div v-else>
      <h1>{{this.characters.name}}</h1>
      <br />
      <br />
      <br />
      <img v-if="this.characters.thumbnail.path" :src="this.characters.thumbnail.path+`/portrait_uncanny.`+this.characters.thumbnail.extension" :alt ="this.characters.name"/>
      <br />
      <br/>
...
export default {
  ...
  methods: {
    ...
    this.characters = data.data.results[0];
    }
      catch(e){
        this.err = true;
      }

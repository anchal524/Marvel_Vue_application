<template>
  <div>
    ...
    <div v-else>
    <ul>
      <li v-for="(ser,index) in series" :key="index" >
        <router-link :to="{name: 'SeriesDetails', params: {id: ser.id}}">Series Name: {{ser.title}}</router-link>
      </li>
    </ul>
...
export default {
  ...
  data() {
    ...
    const baseUrl = "https://gateway.marvel.com:443/v1/public/series"+"?ts=" +ts +"&apikey=" +publickey +"&hash=" +hash +"&offset=" +offset +"&limit=20"; 
    axios
      .get(baseUrl)
      .then(({ data }) => (this.series = data.data.results));
  },getNextPage(){
      let pageNum = (parseInt(this.$route.params.page)+1).toString();
      this.$router.push({name:"series",params:{page:pageNum}})
    } ,getPrevPage(){
      let pageNum = (parseInt(this.$route.params.page)-1).toString();
      this.$router.push({name:"series",params:{page:pageNum}})
    }},
  created() {
if(parseInt(this.$route.params.page) < 645 && parseInt(this.$route.params.page) >= 0){
          if(parseInt(this.$route.params.page) == 0)
          {
            ...
            if(parseInt(this.$route.params.page) == 644)
            {
